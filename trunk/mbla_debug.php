...
if (! function_exists( 'logger' )) {
    ...
    logger( 'mbla[] = ' . serialize( $mbla ) ) ;
    logger( 'mbla_options[] = ' . serialize( $mbla_options ) ) ;
}
...
function id2URL( $service, $id, $md5id ) {
    logger( "id2URL({$service}, {$id}, {$md5id}) {" , + 1 ) ;
    global $mbla, $mbla_options ;
    if (strpos( $id , '@' )) {
        switch ( strtolower( $service )) {
            case 'gravatar' :
                $ret = "http://www.gravatar.com/avatar.php?gravatar_id={$md5id}&rating=X" ;
                break ;

            case 'mybloglog' :
                $ret = "http://pub.mybloglog.com/coiserv.php?href=mailto:{$id}" ;
                break ;
...
function updateNeeded( $id ) {
    logger( "updateNeeded({$id}) {" , + 1 ) ;
    global $mbla, $mbla_options ;
    $update = false ;
    $avatar = "{$mbla['filecache']}/{$id}" ;
    logger( "checking avatar {$mbla['filecache']}/{$id}" ) ;
    ...
    logger( "updateNeeded({$id}) = " . ( int ) $update ) ;
    logger( "}" , - 1 ) ;
    return $update ;
}
...
function fetchAvatar( $INemail = null, $INservice = null ) {
    ...
    $md5id = md5( $id ) ;
    logger( "\$id={$id} | \$md5id={$md5id}" ) ;

    if (updateNeeded( $md5id ) || $INservice) {
        // an update is needed        $services = ($INservice ? array (
            $INservice
        ) : explode( ',' , $mbla_options [ 'prival' ] )) ;
        logger( "\$services " . serialize( $services ) ) ;
        while ( $service = array_shift( $services ) ) {
            if ('none' == $service) {
                ...
                // cycle through the services                switch ( $service) {
                    case 'gravatar' :
                        $remoteFileURL = id2URL( 'gravatar' , $id , $md5id ) ;
                        break ;
                    case 'mybloglog' :
                        $remoteFileURL = id2URL( 'mybloglog' , $id , $md5id ) ;
                        break ;
                    case 'custom' :
                        $remoteFileURL = id2URL( 'custom' , $id , $md5id ) ;
                        break ;
                    case 'mybloglog_anon' :
                    case 'gravatar_anon' :
                        $remoteFileURL = id2URL( 'anon' , $id , $md5id ) ;
                        $INservice = 'localanon' ;
                        break ;
                    case 'none' :
                    default :
                ...
                if (! isAnon( $ret ) || $INservice) {
                    logger( "valid avatar fetched | md5id={$md5id}" ) ;
                    logger( "}" , - 1 ) ;
                    return array (
                        'file' => $ret,
                        'id' => $md5id
        ...
        // no update is needed so just return the md5id        logger( "no update was needed, just return the avatar | md5id={$md5id}" ) ;
        logger( "}" , - 1 ) ;
        return array (
            'file' => $md5id,
            'id' => $md5id
...
if (! function_exists( 'file_put' )) {
     ...
     * (look at file_put vs. file_put_contents and remove one of them!)
     *
     * @param unknown_type $n Full file name to write to
     * @param unknown_type $d String to write
     * @return unknown
    ...
    function file_put( $n, $d ) {
        logger( "file_put({$n}, binary_avatar_data ) {" , + 1 ) ;
        $f = @fopen( $n , "w" ) ;
        if (! $f) {
            $ret = false ;
        } else {
            ...
            fclose( $f ) ;
            $ret = true ;
            logger( "avatar saved in {$bytes} bytes" ) ;
        }
        logger( "}" , - 1 ) ;
        return $ret ;
    }
